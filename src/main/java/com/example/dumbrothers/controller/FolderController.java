...
//    }
    ...
    public ResponseEntity<Folder> create(@RequestBody FolderDto folderDto){
        Folder folder=folderService.save(folderDto);
        return ResponseEntity.status(HttpStatus.OK).body(folder);
    }

    //폴더 id 기준으로 폴더이름 수정
    @PatchMapping("/dum/folder/{id}")
    public ResponseEntity<Folder> update(@PathVariable Long id, @RequestBody FolderDto dto){
...
//
//        List<Link> delLinks = dumService.inshow(id);
//
